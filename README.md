...
ですが、一気にソースコードから、**終端記号**まで変換するのは構造を作れないし、欲しい処理ではないです。
ですので、生成規則は**非終端記号**$\Rightarrow$**非終端記号**または、**非終端記号**$\Rightarrow$**終端記号**などのルールを持っています。また、すべての変換を**非終端記号** $\Rightarrow^*$(0回以上の変換)**終端記号**と、表すこともできます。
